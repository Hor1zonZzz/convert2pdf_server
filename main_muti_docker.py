...
class TempFileHandler(SimpleHTTPRequestHandler):
    """自定义HTTP处理器，提供指定目录的文件服务"""
...
def start_temp_http_server(directory: str, port: int = 0) -> tuple[HTTPServer, int]:
    """
    启动临时HTTP服务器
    返回 (server, actual_port) 的元组
    """
    # 找到一个可用的端口
    if port == 0:
    ...
    # 自定义处理器类
    class CustomHandler(TempFileHandler):
...
# Docker容器管理函数
async def convert_file_with_docker(file_url: str = None, file_path: str = None, task_uuid: str = None) -> tuple[bool, dict]:
    """
    使用Docker容器转换文件为PDF
    支持两种模式：
    1. file_url: 直接传递URL给Docker容器
    2. file_path: 启动临时HTTP服务器提供文件访问
    ...
    try:
        ...
        if file_url:
            ...
            # 启动临时HTTP服务器
            temp_server, server_port = start_temp_http_server(file_dir)
            temp_url = f"http://host.docker.internal:{server_port}/{file_name}"
            logger.info(f"Started temp HTTP server on port {server_port}, file accessible at: {temp_url}")
            ...
            try:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(f"Conversion request failed with status {response.status}: {error_text}")
                        return False, {"error": f"Conversion request failed: {error_text}"}
        ...
        if temp_server:
            ...
            logger.info("Temporary HTTP server shut down")
...
async def convert(request: Request):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Converting file with Docker container, task_uuid: {task_uuid}")

            # 根据输入类型调用不同的Docker转换函数
            if file_url:
                # 模式1：直接传递URL给Docker容器，避免重复下载
                logger.info(f"Using file_url mode for Docker conversion: {file_url}")
                success, response_data = await convert_file_with_docker(file_url=file_url, task_uuid=task_uuid)
            else:
                # 模式2：传递文件路径给Docker容器，启动临时HTTP服务器
                logger.info(f"Using file_path mode for Docker conversion: {download_file_path}")
                success, response_data = await convert_file_with_docker(file_path=str(download_file_path), task_uuid=task_uuid)

            if success:
                # 转换成功，返回Docker服务返回的响应数据
                result.update(response_data)
                logger.info(f"File conversion successful via Docker container, task_uuid: {task_uuid}")
            else:
                # 转换失败
                logger.error(f"Failed to convert file via Docker container, task_uuid: {task_uuid}, error: {response_data}")
                return JSONResponse({"error": response_data.get("error", "Failed to convert file via Docker")}, status_code=500)
        except Exception as e:
            logger.error(f"Failed to convert file via Docker, source: {original_source}, error: {e}")
            return JSONResponse({"error": "Failed to convert file via Docker"}, status_code=500)
